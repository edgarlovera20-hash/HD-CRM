...
const WORKFLOWS: Record<string, { humanReviewRequired: boolean }> = {
  WEB_LEAD_TO_CRM: { humanReviewRequired: false },
  CRM_OVERDUE_ALERT: { humanReviewRequired: true },
  CRM_PAYMENT_COMMITMENT_REMINDER: { humanReviewRequired: true },
};
...
router.post("/", requireN8nSecret, (req, res) => {
  try {
    ...
    console.log(`[AUDIT STUB] n8n webhook workflowId=${workflowId} correlationId=${correlationId} platform=HD-CRM actorType=n8n_workflow`);
    ...
    // Process WEB_LEAD_TO_CRM: create lead from payload
