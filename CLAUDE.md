...
`HD-CRM` is the client-management platform of the Heavenly Dreams (HD) ecosystem
(`crm.heavenlydreams.com.mx`): client records (new/active/overdue), follow-up tasks, conversation
history, approved message templates, payment commitments, and CRM reports. Per
`ecosystem-boundaries.v1.json` it must **not** make hiring decisions, administer global permissions,
own finance treasury, or contact clients autonomously without review. It receives public leads from
`HD-WEB` and forwards events to `HD-BRAIN` (`BRAIN_EVENTS_URL=.../api/events/ingest`).
...
npm run dev          # tsx server.ts — Express server (port 3002)
...
- CRM domain logic and workers also have shared packages in HD-CORE (`@hd/core-*` /
  `crm-domain`, `crm-workers`) — prefer those over duplicating logic.

## RBAC

Use `@hd/core-rbac`'s `hasPermission()` (wildcards like `crm.*`). CRM permissions
(`crm.clients.*`, `crm.followups.create`, `crm.conversations.reply|review`, `crm.reports.view`) are
defined centrally in HD-CORE. Note `crm.conversations.review` exists because autonomous client
contact must be reviewed, not sent blindly.
